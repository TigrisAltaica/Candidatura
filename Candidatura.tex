...
\fxnote*{instancia de que?}{experimentos} se fabricó una nano viga
...
de la cavidad a los cambios en su \fxnote*{ya recordé que puse estructura ya que no todas las cavidades son estilo fabry-perot}{longitud o el equivalente en cavidades de otras geometrías}.
...
\fxnote*{Me parece que tienes que describir con mas cuidado el
  ...
  palabra bomba y usar laser de excitacion o algo asi}{En particular, se estudia un sistema compuesto por una cavidad óptica de Fabry-Perot donde uno de los dos espejos se encuentra acoplado a un oscilador armónico mecánico, lo qual le permite moverse. La frecuencia natural del oscilador mecánico es una función periódica del tiempo. Se asume que el oscilador interactúa únicamente con un modo de la cavidad  con frecuencia $\omega_{cav}$, dicho modo se encuentra forzado por un láser.} Se asume que el marco de referencia rota con la frecuencia del laser de forzamiento. Se modela el sistema mediante el siguiente Hamiltoniano\cite{BarberisLC}
...
de tipo oscilador armónico, \fxnote*{No es la unica solucion. Mas
  ...
  la solución- Explicar mejor que es lo que haces aqui.}{una de las posibles soluciones es} mediante la base de decaimiento\cite{EnglertDB}. Esta solución es particularmente conveniente puesto que más adelante será necesario tomar trazas sobre todos los grados de libertad de la cavidad, lo cual resulta relativamente sencillo en esta base.
...
\begin{figure}[h!]
\includegraphics[scale=.45]{GraficaTemp.pdf} 
...
\chapter{Enfriamiento Optomecánico Dependiente del Tiempo: Cavidad con Disipación Dependiente del Tiempo}

En el capítulo anterior se demostró el profundo efecto que puede tener el considerar una dependencia temporal en la frecuencia natural del oscilador armónico. Este resultado motiva investigar el efecto de dependencias temporales en otros parámetros del sistema. Usualmente, durante las derivaciones para este tipo de sistemas, se asume que la cavidad cuenta con únicamente un modo, el cual tiene  frecuencia constante $\omega_c$. La frecuencia de resonancia de una cavidad de Fabry-Perot, asumiendo que su interior se encuentre en el vacío, está dada por
...
\end{align} donde la expresión se deja en primer orden del parámetro de perturbación $\epsilon = \frac{x_m}{l_0}$ y la dependencia temporal de la amplitud queda contenida en $C(t)$, la cual también contiene un cambio de signo por simplicidad. Claramente la frecuencia natural de la cavidad es una función del tiempo, sin embargo esto tiende a no tomarse en cuenta en la literatura. Como primera aproximación a este problema, consideramos la idea de modificar la disipación de la cavidad, dada por el término ya visto

\begin{align}
L_a \rho =& - \frac{\kappa}{2}(n_p + 1)[a^\dagger a\rho + \rho a^\dagger a -2a\rho a^\dagger]  \\
 &- \frac{\kappa}{2}(n_p)[ aa^\dagger\rho + \rho  aa^\dagger -2a^\dagger\rho a].\nonumber
\end{align} el cual corresponde a una frecuencia natural constante. Cuando se realiza la transformación unitaria de los operadores de desplazamiento, se está tomando como estado de temperatura 0 a un estado coherente, es decir

\begin{equation}
\Ket{\beta(t)}_D = e^{(\beta(t) b^\dagger - \beta(t)^*b)} \Ket{0}.
\end{equation} Esto quiere decir que al enfriar al oscilador armónico mecánico a una temperatura mínima el estado que se obtiene es muy cercano, en el marco de desplazamiento a $\Ket{0}_D$, lo cual es muy cercano a un estado coherente en el marco de referencia inicial. Suponemos entonces que el oscilador armónico mecánico tiende a un estado de tipo

\begin{equation}
\Ket{\beta} + \epsilon\Ket{T}
\end{equation} es decir un estado coherente más una pequeña perturbación procedente de ruido térmico. Proponemos emplear la frecuencia de este estado coherente para determinar la dependencia temporal de la frecuencia de la cavidad. Si el oscilador armónico mecánico se mueve de forma
...
\end{align} Este modelo de disipación debe de ser más correcto, ya que si toma en cuenta que aún cuando se logra enfriamiento del oscilador mecánico, este se continúa moviendo y el largo de la cavidad nunca es constante. Sin embargo, insertar estos operadores en el Hamiltoniano es complicado, ya que estos operadores  no son compatibles con los operadores utilizados para modelar la cavidad. Esto se debe a que los operadores de Floquet se expresan en términos de los operadores de posición y momento de un oscilador con frecuencia natural dependiente del tiempo, mientras que los operadores $b$ y $b^\dagger$ corresponden a un oscilador sin esta dependencia. Si se toma la aproximación de asumir que la frecuencia de la cavidad como su frecuencia promedio y se hace el cambio a operadores de Floquet en el Hamiltoniano, se obtiene un término de la forma
...
\end{equation} donde los coeficientes $C$ dependen de las dos frecuencias y de la forma específica de las soluciones $f(t)$. El tratar de esta forma la dependencia temporal de la cavidad presenta complejidad añadida. Esto señala la necesidad de intentar introducir los operadores de Floquet al Hamiltoniano en primeros principios, después de lo cual estos surgirán de manera natural en la disipación.
...
En este capítulo se presenta una derivación tentativa para lograr modelar la dependencia temporal en la frecuencia natural de la cavidad desde el Hamiltoniano original presentado en \cite{LawOH}.En este caso se trabaja con el enfoque de que la frecuencia natural de la cavidad depende del tiempo. Si se pide que dicha dependencia temporal sea periódica se tiene un sistema de oscilador armónico donde la frecuencia natural es una función periódica del tiempo. Esto permite el uso de los operadores de Floquet.  Se busca un Hamiltoniano  expresado en términos de estos operadores. Se trabaja en el régimen de un solo modo. El Hamiltoniano original de \cite{LawOH} es
...
Esta derivación lleva a un resultado interesante, sin embargo existen problemas de compatibilidad con las ecuaciones originales en \cite{LawOH}. Para entender este problema, se presenta, de forma breve, la derivación original. 
...
\end{equation} Esta expansión equivale a los distintos modos determinados por la posición instantánea del espejo. Uno de los principales objetivos siguientes para este trabajo es buscar otra expansión que permita integrar el formalismo de Floquet desde este punto. Como los modos son un conjunto de funciones completo, se tiene que
...
\end{align} de donde sigue el Hamiltoniano \eqref{LawHamiltonian} de manera directa. El problema de compatibilidad antes mencionado se refiere a si exigir que $q$ realice un movimiento periódico y que $Q$ por ende cumpla la ecuación de Floquet es consistente con \eqref{RegularModesEq}. De no ser así, se buscará encontrar otra expansión de modos que si sea compatible.
