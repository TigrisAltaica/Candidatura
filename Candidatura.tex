...
La optomecánica es el estudio de la interacción entre elementos ópticos y elementos mecánicos. En este capítulo se dará una breve introducción al tipo de sistemas, efectos y aplicaciones que se consideran parte de la optomecánica. Se hace énfasis particular en el enfriamiento optomecánico y como este efecto es fundamentalmente cuántico. Después se explica una variante del enfriamiento optomecánico que motiva este trabajo, el enfriamiento optomecánico con parámetros dependientes del tiempo cuando existe disipación. Los formalismos particulares que se utilizan para modelar este tipo de procesos se introducen en el capítulo \ref{Formalismos}. Los resultados obtenidos en trabajos previos se presentan en el capítulo \ref{ResultadosTesis} y se explica porque estos motivan el trabajo que se propone en el capítulo \ref{PropuestaDoctorado}.	
...
En esta sección se da un pequeño resumen de los efectos 
 más conocidos y utilizados resultantes de la interacción optomecánica. Frecuentemente estos efectos se deben a la interacción entre la presión de radiación que la luz
...
\item \textbf{Enfriamiento Optomecánico} Este es el efecto en el que se basa este trabajo y se describe a mayor detalle más adelante. El efecto consiste en lograr que un sistema de resonador acoplado a una cavidad favorezca las transiciones a estados de menor número de excitaciones por sobre transiciones a estados de mayor número de excitaciones.
\end{itemize}
...
\subsection{Enfriamiento Optomecánico}\label{OptomechanicalCooling}

En esta sección se da una breve explicación de la causa del enfriamiento optomecánico y la razón de que este sea un efecto puramente cuántico. Al hablar de enfriamiento es importante definir que se considera como temperatura. En este trabajo se habla del enfriamiento de un oscilador armónico cuántico, por lo que se toma el número promedio de sus excitaciones como medida de su temperatura. Esta medida se toma para lo que se considera un estado estacionario. El enfriamiento entonces se puede entender como transiciones de estados con un número mayor de excitaciones a estados con un número menor. A continuación se explica de que dependen estas transiciones.  La derivación se basa en la encontrada en \cite{WarwickQO}. Primero es necesario definir la  Densidad de Potencia Espectral

\begin{equation}
S_{hh}(\omega) = \lim_{\tau \to\infty} \frac{1}{\tau}\langle h_\tau^*(\omega)h_\tau(\omega) \rangle ,
\end{equation} donde $h(t)$ una variable estocástica compleja cuyas propiedades estadísticas son
estacionarias. $h_\tau(\omega)$ es la
transformada de Fourier tomada entre $\frac{-\tau}{2}< t <
\frac{\tau}{2}$. Es importante notar que este es un promedio estadístico. La densidad de potencia espectral describe la potencia que se encuentra en una señal. Para el proceso de enfriamiento optomecánico la cantidad de interés será la densidad de potencia espectral de la fuerza ejercida por el baño térmico al que se acopla el oscilador armónico. Para el cálculo será importante expresar la densidad de potencia espectral de una forma específica dada por el siguiente teorema que se presenta sin demostración.
...
\end{center}El teorema es la primera igualdad, la segunda resulta de las propiedades de la transformada de Fourier. En el caso clásico la densidad de potencia espectral es simétrica en la frecuencia. Sin embargo, en el caso cuántico se tiene operadores y esto no es necesariamente cierto. Debido a que, en general, no se puede asegurar que para un operador Hermitiano $\hat{O}(t)$ se tenga $[\hat{O}(t),\hat{O}(t+\tau)] = 0$. Se sigue entonces que no se puede asegurar $S_{OO}(\omega) = S_{OO}(-\omega)$. El hecho de que un operador no necesariamente conmute con si mismo a distintos tiempos rompe la simetría. Se estudia ahora el caso específico de un oscilador armónico cuántico descrito por un Hamiltoniano
...
\begin{equation}
H_0 = \hbar \Omega a^\dagger a + H_{b}
\end{equation}
...
\begin{equation}
V = qF,
\end{equation} es la interacción entre el oscilador y el baño, donde $F$ es la fuerza ejercida por el baño, la cual conmuta con $q$. $H_0$ es el Hamiltoniano usual para un oscilador armónico cuántico más $H_{b}$, el Hamiltoniano que describe el baño térmico al que está acoplado el oscilador. Se desea obtener la probabilidad de que el oscilador armónico pase de un estado inicial $\Ket{\Psi(t)}$  a un estado final ortogonal $\Ket{\Psi_f(t)}$. Se trabaja en el cuadro de interacción por simplicidad. La amplitud de transición $A_{if}(t)$ está dada por
...
\end{align} En el cuadro de interacción el Hamiltoniano está dado por

\begin{align}
H_I(t) &= U_0^\dagger(t)HU_0(t), \\
&= U_0^\dagger(t)VU_0(t), \\
&=V_I(t).
\end{align} Utilizando teoría de perturbación a primer orden, y asumiendo que la interacción es débil y por lo tanto los estados del baño y del oscilador armónico se mantienen separables, se llega a que la amplitud de transición para pasar de un estado $n$ a un estado $n+1$ en el oscilador, dado que el baño inicia en algún estado $j$ y que la interacción lo deje en un estado $k$, es

\begin{equation}
A_{if}(t) = \frac{x_{zp}\sqrt{n+1}}{i\hbar}\int_0^t d\tau_1 e^{i\Omega \tau_1} \Bra{k}F(\tau_1)\Ket{j},
\end{equation} donde $F_I(t)$ es la fuerza que ejerce el baño en el cuadro de interacción y $x_{zp}$ es la amplitud mínima del oscilador. La probabilidad de transición se obtiene al sumar sobre los cuadrados de las amplitudes de probabilidad correspondientes a todos los estados finales posibles

\begin{equation}
P_{n \to n+1} = \sum_k |A_{if}(t)|^2 = \frac{x_{zp}^2 (n+1)}{\hbar^2} \int\int_0^t d\tau_1 d\tau_2 e^{i \Omega (\tau_2-\tau_1)}\langle F_I(\tau_1)F_I(\tau_2) \rangle ,
\end{equation} donde se ha utilizado que $F(t)$ es un operador Hermitiano y que los modos del baño son completos. Se utilizan dos cambios de variable $\tau_1 = t' + \tau$ y $\tau_2 = t'$ para hacer más evidente la relación con \eqref{WienerKhichin}.
...
\end{equation} Las transiciones que aumentan el número de excitaciones dependen de la parte negativa del espectro $S_{FF}$ mientras que las transiciones que reducen el número de excitaciones dependen de la parte positiva.

En el caso clásico $S_{FF}(\Omega)$ es una función simétrica, sin embargo, esta simetría se pierde al reemplazar variables por operadores debido a las relaciones de conmutación. En el caso clásico las tazas de transición hacia estados más y menos excitados son básicamente iguales, pero en el caso cuántico esto ya no es cierto. Si se encuentra el régimen de parámetros adecuados, se puede colocar el sistema de tal forma que una de las dos tazas sea mayor a la otra y así lograr un sistema que logre calentamiento o enfriamiento. Este efecto es intrínsecamente cuántico. 
 

\section{Enfriamiento Optomecánico con Parámetros Dependientes del Tiempo}

Este trabajo se enfoca en el enfriamiento optomecánico cuando existe disipación. Uno de los métodos empleados en la búsqueda por mejorar el enfriamiento de un oscilador mecánico acoplado a una cavidad es utilizar un oscilador mecánico cuya frecuencia natural sea función del tiempo \cite{BarberisLC}. Esta dependencia modifica el
cuasi espectro de energía del sistema\cite{HanngiFM}. En el formalismo de disipación
empleado en \cite{BarberisLC} esto no se toma en cuenta, el efecto de la dependencia temporal sobre la disipación se modela mediante coeficientes con dependencia temporal que se introducen de manera ad-hoc.  Sin embargo, \cite{HanngiFM} muestra que este no es el enfoque óptimo, así que se realizó un trabajo que sí toma en cuenta los efectos de la dependencia temporal de la frecuencia natural del oscilador durante la derivación
de la temperatura final que se espera del sistema \cite{YanesOC} y esta dependencia queda codificada en los operadores tanto del Hamiltoniano como los de los términos de disipación e interacción. Los resultados de este trabajo muestran diferencias cuantitativas y
cualitativas en el comportamiento de la temperatura del oscilador mecánico, lo cual motiva la pregunta ¿Qué sucede al tomar en cuenta la dependencia temporal de la frecuencia natural de la cavidad en el modelo de disipación?
...
obtiene la temperatura final del sistema para el caso del oscilador paramétrico. Finalmente se propone aplicar este formalismo a un sistema donde se toma en cuenta la
...
\chapter{Oscilador Armónico Dependiente del Tiempo y Sistemas Cuánticos Abiertos}\label{Formalismos}

En un trabajo previo \cite{TesisMaestria} se encontró que cuando la frecuencia natural de un oscilador armónico mecánico con fricción acoplado a una cavidad óptica es una función periódica del tiempo, y el modelo de disipación toma en cuenta esta dependencia, se puede enfriar el oscilador a una temperatura menor que en el caso donde la frecuencia natural es constante. Como ya se mencionó la sección \ref{OptomechanicalCooling}, por temperatura se entiende el número promedio de excitaciones del oscilador. Para poder entender este resultado es necesario poder modelar un oscilador armónico dependiente del tiempo en mecánica cuántica y poder modelar sistemas cuánticos abiertos, es decir con intercambios de energía con su ambiente. En este capítulo se discuten ambos formalismos. 

Para  modelar el oscilador armónico cuántico dependiente del tiempo de forma que su dependencia temporal pueda ser tomada en cuenta en el modelo de disipación es extremadamente útil emplear un formalismo que codifique la dependencia temporal dentro de los operadores utilizados. Esto se debe a que se ha demostrado \cite{HanngiFM} que el modelo de disipación es mejor si se utilizan estos operadores. Este formalismo se conoce como teoría de Floquet. Esta teoría permite obtener soluciones para las ecuaciones clásicas de movimiento del oscilador armónico dependiente del tiempo, las cuales se utilizan posteriormente en la solución del caso cuántico. 

Una vez establecida la forma de modelar el oscilador armónico dependiente del tiempo se procederá a dar una breve explicación de como modelar sistemas cuánticos con disipación. Se dará una explicación del formalismo de ecuaciones maestras y de la base que se emplea para trabajar el caso específico de un oscilador armónico cuántico con disipación. 

\section{Teoría de Floquet}

El primer tema a tratar es el formalismo empleado para tratar el oscilador armónico dependiente del tiempo en el caso clásico. Se desea resolver una ecuación diferencial que involucra coeficientes con dependencia temporal, tal como
...
A manera de ejemplo se obtienen las soluciones para un caso específico del oscilador armónico dependiente del tiempo. Estas soluciones se utilizarán más adelante para resolver el caso cuántico. Se toma como función periódica una función constante más una pequeña perturbación periódica
...
\end{equation} y su complejo conjugado, el cual es $f(-t)$.

Armados con una solución particular para el caso clásico, es posible proceder a estudiar como se resuelve el caso cuántico.

\section{Estados de Floquet en Mecánica Cuántica}

Utilizaremos los resultados obtenidos en la  sección anterior para estudiar un Hamiltoniano con un parámetro con una dependencia periódica en el tiempo
...
\end{equation} La forma explícita de estos estados y de los operadores que se utilizan para describir el Hamiltoniano dependen, como ya se mencionó anteriormente, de las soluciones al caso clásico, como las obtenidas en la sección \ref{SmallOscilations}. En la siguiente sección se explica como entran estas soluciones clásicas al caso cuántico.
...
Se tiene un Hamiltoniano que modela un oscilador armónico cuántico cuya frecuencia natural depende del tiempo. Tanto los operadores como la constante de escala dependen explícitamente de las soluciones al caso clásico del oscilador armónico dependiente del tiempo. A continuación se explica como se modela el sistema cuando este tiene disipación y se explica porque es de tanta importancia que la dependencia temporal del sistema quede codificada en los operadores que lo describen. 

\section{Sistemas Cuánticos Abiertos}\label{Disipacion}

Las ecuaciones maestras modelan la evolución temporal de un sistema donde puede haber intercambios de energía con el medio ambiente, al cual se le conoce usualmente como reservorio o baño térmico. Se busca modelar un oscilador armónico
mecánico con fricción, cuya frecuencia natural depende del tiempo, e
intercambia energía con una cavidad electromagnética con perdidas de energía. Para una derivación de este tipo de ecuaciones, el lector puede consultar\cite{ZollerQN}. La ecuación es muy similar a la ecuación de Liouville, ya que se estudia la evolución temporal de la matriz densidad del sistema. Normalmente este tipo de ecuaciones requieren de dos aproximaciones. La aproximación de Born donde se asume que la interacción con el reservorio es lo suficientemente débil como para descartar términos de mayor a segundo orden en la misma y la aproximación de Markov, donde se asume que el reservorio térmico no cambia su estado debido a su interacción con el resto del sistema y de esta forma no actúa como un tipo de memoria para este\cite{CarmichaelFL}. Estas ecuaciones tienen la forma general

\begin{equation}\label{MasterEq}
\dot{\rho} = \frac{1}{i\hbar}[H,\rho] + L\rho,
\end{equation} donde $\rho$ es la matriz densidad del sistema, $H$ es el Hamiltoniano que modela el sistema sin tomar en cuenta intercambios de energía con un reservorio y $L$ es el superoperador de Lindblad, el cual codifica el efecto del reservorio sobre el sistema, es decir la parte del sistema que puede interaccionar con el medio ambiente y causar intercambios de energía. La forma de $L$ depende de la forma de $H$ y queda expresado en los mismos operadores que $H$ por lo que la elección de operadores de $H$ determina la forma de $L$.
...
\end{equation}  En el caso de un  oscilador armónico con pérdidas, se obtiene un conjunto de estados y otro de estados duales que sí forman una base del espacio \cite{EnglertDB}. Los estados izquierdos son $\rho_n^l$, los cuales corren sobre dos índices discretos y corresponden a los valores propios
\cite{EnglertDB}
...
\end{equation}es importante notar que la parte real de esos valores nunca es positiva.

Este modelo de disipación explica la necesidad de emplear los operadores de Floquet. En \cite{BarberisLC} se emplean los operadores usuales del oscilador armónico cuántico bajo la aproximación de que si las variaciones en la frecuencia son pequeñas respecto a la frecuencia promedio se pueden tomar los operadores correspondientes a la frecuencia promedio. Luego se toma en cuenta la dependencia temporal del sistema utilizando un coeficiente con dependencia temporal en el término de Lindblad. Sin embargo, \cite{HanngiFM} demuestra que este no es el mejor modelo. Para poder tomar en cuenta la dependencia temporal del sistema en el modelo de disipación es necesario codificar esta dependencia en los operadores del Hamiltoniano. A continuación se explican los resultados obtenidos en \cite{TesisMaestria} donde se emplean las técnicas desarrolladas en este capítulo para buscar una expresión para la temperatura final del oscilador armónico después del proceso de enfriamiento.  



\chapter{Enfriamiento Optomecánico Dependiente del Tiempo: Caso de Oscilador Armónico Dependiente del Tiempo}\label{ResultadosTesis}

En este capítulo se da un breve resumen del trabajo realizado en \cite{TesisMaestria} y en \cite{YanesOC}. Se modela un oscilador armónico mecánico acoplado a una cavidad de Fabry-Perot cuando se toma en cuenta una dependencia temporal en la frecuencia natural del oscilador. Se obtienen cambios cuantitativos y cualitativos en el enfriamiento del oscilador cuando se utiliza un modelo de disipación que incorpore esta dependencia temporal. Primero se presenta el Hamiltoniano utilizado para describir al sistema y la ecuación maestra correspondiente. Esta ecuación se transforma a otro marco de referencia más conveniente y finalmente se lleva acabo el proceso de enfriamiento bajo la aproximación adiabática a fin de obtener una expresión para la temperatura final. Esto motiva el trabajo que inicia en el capítulo siguiente.
...
Sin embargo el Hamiltoniano no es suficiente para modelar el sistema ya que existe disipación, se utiliza el formalismo de ecuaciones maestras de la sección \ref{Disipacion} a fin de modelar esta disipación. Se utiliza un modelo de disipación que incorpora los operadores de Floquet y por ende la dependencia temporal del oscilador durante todo el proceso de derivación 

\begin{equation}
\dot{rho} = \frac{1}{i\hbar}[H(t),\rho] + L_a \rho + L_\Gamma \rho
\end{equation}

Sin embargo es complicado trabajar con esta ecuación debido a que el término de interacción del Hamiltoniano contiene términos de tercer orden en operadores. Para eliminar estos términos se emplea una transformación unitaria a un marco de referencia desplazado. En la siguiente sección se realiza la transformación de forma detallada.


\section{Transformación Mediante Operador de Desplazamiento}\label{Desplazamiento}

Para poder remover los términos de tercer orden en operadores del Hamiltoniano, se emplea una transformación unitaria dada por el operador
...
Para poder encontrar expresiones explícitas para los coeficientes restantes, es necesario tener expresiones explícitas para la solución clásica al problema del oscilador armónico dependiente del tiempo, ya que estas determinan la expresión exacta de los operadores $\Gamma$ entre otras cosas. Se utilizan las dos soluciones encontradas en la sección \ref{SmallOscilations}.

\subsection{Solución Explícita para Pequeñas Oscilaciones }

Con una solución para el caso de una frecuencia como en\eqref{SmallOscillationsTDHO}  en mano, es posible calcular soluciones explícitas para todos los términos que se han obtenido. Estos resultan ser\cite{TesisMaestria}
...
Ahora el Hamiltoniano tiene la forma adecuada para realizar el procedimiento de enfriamiento y para obtener una expresión para la temperatura final del oscilador armónico.
...
Q\rho=&(1-P)\rho,
\end{align} donde $\rho^0$ es el estado inicial del baño térmico. El propósito de estos dos proyectores es separar los sub-espacios de acuerdo con las escalas de tiempo en la que operan y llegar a una ecuación cerrada para el sub-espacio $P$ 
...
El número promedio de excitaciones, el cual se utiliza como una medida de temperatura, está dado por

\begin{equation}
<\Gamma^\dagger \Gamma> = <m>  = \frac{A_+}{A_- - A_+}
\end{equation} donde $A_-$ y $A_+$ son los coeficientes de enfriamiento y calentamiento respectivamente. Estos coeficientes se obtienen a partir de \eqref{CavityQuadrature} y tienen la forma

\begin{equation}
A_{\pm \nu}(n_p) = g^2Real(G(\mp \nu,n_p)).
\end{equation} Por lo tanto es necesario obtener una expresión explícita para \eqref{CavityQuadrature} para poder obtener una expresión explícita para $<m>$. Esta expresión resulta difícil de analizar de forma directa, así que se recurre a métodos numéricos.
...
por $\lambda_c$ . El resultado de la traza en \eqref{CavityQuadrature} se calcula de manera directa y queda calcular la integral
...
Es importante notar que el desarrollo en serie es posible ya que la parte real de $\lambda_c$ es negativa, lo cual evita que la integral diverja. Esto lleva a la expresión usual para \ref{CavityQuadrature}, cuando no existe una dependencia temporal, más un término proporcional a $\epsilon$ que contiene el efecto de la dependencia temporal de la frecuencia
...
analítica, pero se puede analizar numéricamente. A fin de que sea válida la aproximación
...
Tomando como ejemplo un valor inicial de $\epsilon = \frac{\nu_0}{10}$ y debido a \eqref{scattering} se tiene la condición
$\epsilon = \frac{n^2}{5}$. Dado que se desea que $n$ sea grande se
elige $n=\sqrt{\frac{\nu_0}{2}}$ redondeada al entero más cercano.


El cálculo numérico de \eqref{GWithCorrection} utiliza la proporción entre la frecuencia promedio $\nu_0$ y
el desfazamiento $\delta$ en el rango
$\frac{\nu_0}{\delta} \in [-2,2]$ como variable dependiente. Se obtiene

\begin{figure}[ht!]
\includegraphics[scale=.7]{GraficaTemp.pdf} 
\caption{\textit{Comparación entre predicciones con dependencia temporal y sin dependencia temporal} La línea puntuada representa la predicción sin dependencia temporal y la línea sólida representa la predicción con dependencia temporal.  Se utiliza $\kappa \ll \nu_0$}
\label{GraficaEnfriamiento}
\end{figure}

Como se observa en la figura \ref{GraficaEnfriamiento} claramente se obtiene un mínimo más pequeño de excitaciones posibles y un corrimiento en el punto donde se espera encontrar este mínimo. Esto correspondería a un mayor enfriamiento del espejo, pero no en el punto donde se esperaría encontrar sin la corrección. Introducir una dependencia temporal a la frecuencia natural del oscilador y realizar el proceso de enfriamiento cuando el modelo de disipación toma en cuenta la dependencia temporal lleva a cambios cuantitativos y cualitativos en la temperatura final del oscilador armónico. Como se verá en el siguiente capítulo, la frecuencia de la cavidad a la que se acopla el oscilador tiene una dependencia temporal inherente, la cual se modela usualmente de tal forma que la disipación de la cavidad no toma en cuenta esta dependencia. Se desea estudiar que sucede si se cambia este modelo para tomar en cuenta esta dependencia.

\chapter{Enfriamiento Optomecánico Dependiente del Tiempo: Modelo Mejorado de Disipación Aplicado a Cavidad}\label{PropuestaDoctorado}
...
La frecuencia natural de los modos de la cavidad en el estado
  ...
  del campo electromagnético a través de los espejos de la cavidad.
...
de algún estado térmico. Proponemos emplear la frecuencia de este estado coherente en el modelo mejorado para el campo dentro de la cavidad.
...
\begin{figure}
\includegraphics[scale=.35]{OptomechanicalCooling.pdf}
\caption{Campo electromagnético dentro de una cavidad donde uno de los espejos se encuentra acoplado a un resorte. La cavidad tiene longitud total $L$ y se asume que el espejo que se mueve está confinado a moverse en el rango $x_m$. Hay un láser de bombeo con frecuencia $\Omega$ y un resorte de constante $K$. El decaimiento de la cavidad está dado por $\kappa$ y el del resorte por $\gamma$. }
\end{figure}
...
\end{equation} Donde $p$ y $q$ corresponden al oscilador mecánico y $P$ y
$Q$ al campo dentro de la cavidad  y $g$ es una constante. Este es el Hamiltoniano usual para modelar este tipo de oscilador optomecánico. El procedimiento usual después después este punto consiste en cuantizar, e incluye una aproximación lineal para eliminar la dependencia de $q$ sobre los operadores que modelan la cavidad. Al querer modelar la disipación, los operadores de la cavidad ya no contienen la dependencia temporal de la frecuencia de la cavidad, expresada en su dependencia sobre $q$. Por lo tanto, si se busca utilizar los resultados obtenidos anteriormente para el oscilador armónico, es necesario implementar el formalismo de operadores $\Gamma$ pero ahora para modelar la cavidad.

\section{Introducción Directa de Operadores $\Gamma$ en el Hamiltoniano}
...
\item Llegar a una expresión para la temperatura final del espejo bajo
  ...
  se obtiene al no considerar  el movimiento de la
    cavidad en el modelo de disipación  Esto requiere deducir desde primeros principios la  ecuación maestra, tomando en cuenta explícitamente una base
...
Se aplica la transformación unitaria al marco desplazado al Hamiltoniano donde la cavidad está expresada en operadores $\Gamma$. 
