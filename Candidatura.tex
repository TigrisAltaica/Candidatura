...
\chapter*{Objetivo General}
 En un trabajo anterior se estudió el enfriamiento optomecánico de un oscilador armónico mecánico acoplado a una cavidad de Fabry-Perot cuando la frecuencia natural de este es una función periódica del tiempo. Se ha demostrado que utilizar un formalismo de disipación que toma en cuenta la dependencia temporal de la frecuencia natural del oscilador mecánico lleva a un mejor modelo teórico para el
sistema\cite{HanngiFM}. Se encontró que emplear este formalismo de disipación lleva a una predicción cualitativa y cuantitativamente distinta para la temperatura final del espejo. Esto motiva la pregunta: ¿Puede escribirse el Hamiltoniano del sistema a forma de usar este modelo para la disipación en el caso de la cavidad? En este trabajo se
investiga esta posibilidad.
...
En este capítulo se presenta una derivación tentativa para re-escribir el Hamiltoniano original presentado en \cite{LawOH}. En este caso se trabaja con el enfoque de que la frecuencia natural de la cavidad depende del tiempo. Si se pide que dicha dependencia temporal sea periódica se tiene un sistema de oscilador armónico donde la frecuencia natural es una función periódica del tiempo. Esto permite el uso de los operadores de Floquet. Primero se presenta la derivación original del Hamiltoniano a fin de aclarar el procedimiento.
...
Esta derivación lleva a un resultado interesante, sin embargo existen problemas de compatibilidad con las ecuaciones originales en \cite{LawOH}. El problema se refiere a si exigir que $q$ realice un movimiento periódico y que $Q$ por ende cumpla la ecuación de Floquet es consistente con \eqref{RegularModesEq}. De no ser así, el siguiente paso es buscar una expansión para los modos $Q$ que sí sea compatible con los operadores de Floquet.
...
A partir del Hamiltoniano \eqref{LCHamiltonian} es posible estudiar un sistema de gran importancia tomando en cuenta su dependencia temporal inherente, la cual se no se toma en cuenta en el Hamiltoniano usual\cite{CavesIF}. A partir de este punto se proponen tres objetivos para realizar un estudio más completo de este sistema bajo este nuevo enfoque. En todos los casos se espera obtener expresiones analíticas y resultados numéricos  para la temperatura final del oscilador mecánico bajo la aproximación adiabática.
